...
class DigitalFilterDesigner(QMainWindow, Ui_MainWindow):
    ...
    def init_UI(self):
        ...
        # self.pair_mode_toggle.isChecked().connect(self.pair_mode)
    ...
    def on_click(self, event):
        if event.button() == Qt.LeftButton:
            if event.modifiers() == Qt.ControlModifier:
                ...
                self.add_point(self.mouse_loc_circle, self.added, True)
                self.update_plot()
            else:
    ...
    def add_point(self, point, dict, pair_mode=False):
    ...
    def get_all_pass_filter(self):
        ...
        for row in range(self.table_coeff.rowCount()):
            ...
            if item.checkState() == Qt.CheckState.Checked:
                self.checked_coeffs.append(complex(item.text()))  
    ...
    def filter_data(self,data):
        ...
        subset = np.real(lfilter(numerator, denominator, data))
        self.data_modified.append(subset[-1])
    ...
    def on_mouse_move(self, pos):
        if  self.mouse_enable:
            ...
            # signal_value = pos.y()**2

            self.counter_max += 1

            self.order = len(self.data_dict["Zeros"]) + len(self.data_dict["Poles"])
            # Append the signal value to self.data
            self.data.append(signal_value)

            data_filter = self.data[len(self.data)-self.order:]
            # Update the filtered data
            self.filter_data_point_by_point(data_filter)
    ...
    def filter_data_point_by_point(self,data):
